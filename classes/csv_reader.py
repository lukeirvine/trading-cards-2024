    ...
    def _check_data(self, staff_members):
        ...
        for sm in staff_members:
            ...
            if not os.path.exists('images/' + sm.image_path):
                errors.append(f"IMAGE ERROR {i + 2}: The image \"{sm.image_path}\" for {sm.name} doesn't exist. Check line {i + 2} in the csv file.")
            ...
            if int(sm.years_worked) > MAX_YEARS:
                errors.append(f"The years worked for {sm.name} is greater than {MAX_YEARS}.")
        ...
        if len(errors) > 0:
            error_str = "\n".join(errors)
            raise RuntimeError(f"There were {len(errors)} errors with your data:\n" + error_str)
